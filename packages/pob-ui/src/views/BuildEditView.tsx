...
type LoadState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; buildKey: string; summary: PobBuildSummary }
...
const scheduleHiddenModePreload = (callback: () => void): (() => void) => {
...
>(
  (
    ...
    const [buildNameActionError, setBuildNameActionError] = useState<
      string | null
    >(null);
    const [modePreloadState, setModePreloadState] = useState<{
      sessionKey: string;
      modes: Set<BuildMode>;
    }>({ sessionKey: "", modes: new Set() });
    ...
    const hasActivatedPassiveTreeRef = useRef(false);
    const repoeTranslationsCacheRef = useRef(
      new Map<PobRepoeLocale, PobRepoeTranslationsSnapshot>(),
    );

    const buildName = fileName ?? t("buildEdit.empty.title");
    const isUserDraft = fileName === null && draftKey > 0;
    const buildTargetKey = createBuildEditTargetKey({
      subPath,
      fileName,
      draftKey,
    });
    const passiveTreeSessionKey = createBuildEditSessionKey(
      buildTargetKey,
      sessionRevision,
    );
    const ready =
      loadState.status === "ready" && loadState.buildKey === buildTargetKey;
    const displayLoadStatus =
      loadState.status === "ready" && !ready ? "loading" : loadState.status;
    const sessionPreloadedModes =
      modePreloadState.sessionKey === passiveTreeSessionKey
        ? modePreloadState.modes
        : null;
    const isModePreloaded = (mode: BuildMode): boolean =>
      sessionPreloadedModes?.has(mode) ?? false;
    const shouldMountModePanel = (mode: BuildMode): boolean =>
      activeMode === mode || isModePreloaded(mode);
    ...
    useEffect(() => {
      let cancelled = false;
      const expectedBuildKey = buildTargetKey;
    ...
    }, [ready, sessionRevision]);

    useEffect(() => {
      const locale: PobRepoeLocale =
        i18n.resolvedLanguage === "en" ? "en" : "ko";
      const cached = repoeTranslationsCacheRef.current.get(locale);

      if (!ready) {
        setRepoeTranslations(cached ?? EMPTY_REPOE_TRANSLATIONS);
        return;
      }

      if (cached) {
        setRepoeTranslations(cached);
        return;
      }
      ...
      const loadTranslations = async () => {
        ...
        if (cancelled) return;
        if (result.status === "ok") {
          repoeTranslationsCacheRef.current.set(locale, result.snapshot);
          setRepoeTranslations(result.snapshot);
        } else {
          setRepoeTranslations(EMPTY_REPOE_TRANSLATIONS);
        }
      };
    ...
    useEffect(() => {
      ...
      const loadMainSkillSummary = async () => {
        ...
        if (result.status === "ok") {
          onMainSkillSummaryChange({
            status: "ready",
            snapshot: translateMainSkillSummarySnapshot(
              result.snapshot,
              repoeTranslations,
            ),
          });
    ...
    useEffect(() => {
      if (!ready || activeBuildAction !== "importExport") {
    ...
    }, [activeBuildAction, ready, sessionRevision, t]);
    ...
    const summary =
      ready && loadState.status === "ready" ? loadState.summary : null;
    const displaySummary = useMemo(
      () =>
        summary ? translateBuildSummary(summary, repoeTranslations) : null,
      [repoeTranslations, summary],
    );
    const buildMetadata =
      ready && buildMetadataState.status === "ready"
        ? buildMetadataState.snapshot
        : null;
    const displayBuildMetadata = useMemo(
      () =>
        buildMetadata
          ? translateBuildMetadataSnapshot(buildMetadata, repoeTranslations)
          : null,
      [buildMetadata, repoeTranslations],
    );
    const ascendancyOptions = useMemo(
      () => resolveBuildMetadataAscendancies(displayBuildMetadata),
      [displayBuildMetadata],
    );
    ...
    const summaryStats = useMemo(
      () => [
        {
          ...
          value: present(displaySummary?.mainSkillName),
        },
        {
      ...
      [displaySummary?.mainSkillName, summary, t],
    );
    ...
    useEffect(() => {
      hasActivatedPassiveTreeRef.current = false;
    }, [buildTargetKey]);

    useEffect(() => {
      if (activeMode === "tree") {
        hasActivatedPassiveTreeRef.current = true;
      }
    }, [activeMode, buildTargetKey]);
    useEffect(() => {
      if (
        !ready ||
        uiMode === "legacy" ||
        activeMode === "tree" ||
        hasActivatedPassiveTreeRef.current
      ) {
        return;
      }
      const api = window.pobAPI;
    ...
    useEffect(() => {
      if (!ready || uiMode === "legacy") return;
      const modes = getBuildModePreloadOrder(activeMode).filter(
      ...
      const addMode = (mode: BuildMode) => {
        setModePreloadState((prev) => {
          const modesForSession =
            prev.sessionKey === passiveTreeSessionKey
              ? new Set(prev.modes)
              : new Set<BuildMode>();
          if (modesForSession.has(mode)) return prev;
          modesForSession.add(mode);
          return { sessionKey: passiveTreeSessionKey, modes: modesForSession };
        });
      };
      const scheduleNext = (index: number) => {
        if (cancelled || index >= modes.length) return;
        cancelScheduled = scheduleHiddenModePreload(() => {
          if (cancelled) return;
          addMode(modes[index]);
    ...
    }, [activeMode, passiveTreeSessionKey, ready, uiMode]);
    ...
    const renderRenewedModePanel = (mode: BuildMode) => {
      if (!shouldMountModePanel(mode)) return null;
      const visible = activeMode === mode;
      const preload = isModePreloaded(mode);
      if (mode === "tree") {
        return (
          <PassiveTreeView
            active={visible}
      ...
      <div className="pob-edit">
        <div className="pob-edit-header">
          <div className="pob-edit-title-block">
            ...
            />
            <p>
              {displayLoadStatus === "loading"
                ? t("buildEdit.loading")
                : displayLoadStatus === "idle"
            ...
            <div className="pob-build-metadata-controls">
              ...
              <label className="pob-build-metadata-field">
                ...
                >
                  {!buildMetadata && (
                    <option value="">
                      {present(displaySummary?.className)}
                    </option>
                  )}
                  {displayBuildMetadata?.classes.map((classOption) => (
              ...
              <label className="pob-build-metadata-field">
                ...
                >
                  {!buildMetadata && (
                    <option value="">
                      {present(displaySummary?.ascendClassName)}
                    </option>
                  )}
        ...
        <div className="pob-mode">
          ...
          <div
            className="pob-mode-panel"
            role="tabpanel"
            key={passiveTreeSessionKey}
          >
            {!ready ? (
              <p className="pob-mode-placeholder-body">
              ...
              POB_BUILD_MODES.map((mode) => (
                <div
                  key={mode}
                  className="pob-mode-panel-slot"
                  data-pob-mode={mode}
                  hidden={activeMode !== mode}
                >
                  {renderRenewedModePanel(mode)}
                </div>
              ))
            )}
          </div>
                      ...
                      {importExportSnapshot && (
                        <div className="pob-build-share-row">
                          ...
                          <PobUnimplementedButton
                            className="pob-edit-action"
                            controlId="import-export.share-url"
                            ...
                            onNotice={showImportExportUnavailable}
                          >
                            {
                    ...
                    importExportSnapshot && (
                      <section className="pob-build-share-section is-compact">
                        ...
                        <div className="pob-build-share-row">
                          <PobUnimplementedButton
                            className="pob-edit-action"
                            controlId="import-export.character-auth"
                            notice={t("buildEdit.unimplemented.notice", {
                              reason: t(
                        ...
                        <div className="pob-build-share-row">
                          <PobUnimplementedButton
                            className="pob-edit-action"
                            controlId="import-export.character-tree"
                            notice={t("buildEdit.unimplemented.notice", {
                              reason: t(
                                "buildEdit.importExport.error.characterUnsupported",
                              ),
                          ...
                          <PobUnimplementedButton
                            className="pob-edit-action"
                            controlId="import-export.character-items"
                            notice={t("buildEdit.unimplemented.notice", {
                              reason: t(
                                "buildEdit.importExport.error.characterUnsupported",
                              ),
