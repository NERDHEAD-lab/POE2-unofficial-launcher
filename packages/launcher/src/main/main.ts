...
import { DEBUG_APP_CONFIG } from "@poe2-launcher/shared/config";
import {
  getGameName,
  getLauncherTitle,
  getAppName,
} from "@poe2-launcher/shared/naming";
import {
  AppConfig,
  RunStatus,
  NewsCategory,
...
ipcMain.handle("file:get-hash", async (_event, filePath: string) => {
  try {
    ...
    if (filePath.startsWith("file://")) {
      ...
      // In dev mode, assets are served from /packages/launcher/src/renderer/assets or /public
...
app.whenReady().then(async () => {
  ...
  setPobBridgeLogger(logger);
  registerPobLauncherHandlers();
  registerBuildsHandlers();
  registerPobSessionHandlers({ installLocator: getPobInstallPath });
