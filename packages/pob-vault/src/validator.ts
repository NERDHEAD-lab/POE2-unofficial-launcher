...
import type { PobBuildSummary } from "@poe2-launcher/shared/types";
...
export interface PobVaultSmokePingResult {
  pong: boolean;
  pobVersion: string;
}

export interface PobVaultSmokeExportBuildXmlResult {
  xml: string;
}

export interface PobVaultSmokeSession {
  ping(): Promise<PobVaultSmokePingResult>;
  loadBuildXml(xml: string, name?: string): Promise<PobBuildSummary>;
  exportBuildXml(): Promise<PobVaultSmokeExportBuildXmlResult>;
