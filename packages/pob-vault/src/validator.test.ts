...
import { encodePobBuildCodeXml } from "@poe2-launcher/pob-repoe/buildCode";
...
describe("PobVaultContractValidator", () => {
  ...
  it("passes the staged vault path into the injected session factory", async () => {
    const session = makeSession();
    ...
    await validator.runSmokeTest(tempRoot);

    expect(sessionFactory).toHaveBeenCalledWith(tempRoot);
  });
