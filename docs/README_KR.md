...
**Path of Exile** 및 **Path of Exile 2** (한국 서버)의 실행 과정을 자동화하기 위해 설계된 일렉트론(Electron) 기반 커스텀 런처입니다. 로그인, 지정 PC 확인, 인트로 팝업 등을 자동으로 처리하여 쾌적한 게임 실행 환경을 제공합니다.
...
- **듀얼 윈도우 아키텍처**:
  ...
  - **배경 게임 윈도우**: 실제 다음 게임 스타터 웹 프로세스를 백그라운드에서 처리합니다 (디버그 모드 시 노출 가능).
