...
### 2026-07-30 자동 업데이트 기본 선택

- 리뉴얼 예약의 `업데이트 감지 후` 선택 순서를
  `자동 업데이트 | 알림만`으로 바꾸고 자동 업데이트를 초기 기본값으로
  지정했다.
- 모달을 닫았다 다시 열 때도 자동 업데이트로 초기화하며, 한 번 예약에서는
  `업데이트 완료 후 게임 실행` 옵션이 기본 화면에 바로 노출된다.
- `[Windows-pwsh]` 대상 모달 테스트 11개, 전체 ESLint, TypeScript, Vite
  renderer/main/preload 빌드가 통과했다.
- `[Windows-pwsh]` 전체 테스트는 58개 파일 중 57개, 316개 테스트 중 315개가
  통과했다. 기존 외부 GGG POE1 공지 본문이 32자로 짧아진 라이브 통합 테스트
  1건만 동일하게 실패했다.
- `[Windows hidden Electron]` 실제 UI에서 `자동 업데이트 | 알림만` 순서,
  자동 업데이트 선택, 게임 실행 옵션 노출을 확인했다. 알림만으로 전환하면
  게임 실행 옵션이 사라지고 모달 재오픈 시 자동 업데이트로 복원됐다.
  fatal 화면과 runtime exception은 없었으며 전용 프로세스 트리, 54121/9334
  포트와 임시 user-data 프로필을 정리했다.

### 리뷰 라운드 19

판정: `통과 — blocker 없음`

- 최초 state와 모달 open 초기화를 모두 자동 업데이트로 바꿔 재오픈 상태가
  일치하고, 버튼 DOM 순서·선택 조건·클릭 핸들러도 같은 순서로 연결됐다.
- 한 번 예약은 기존 payload 분기에서
  `{ kind: "auto-update", launchAfterUpdate: false }`를 생성해 게임 실행을
  강제로 켜지 않는다. 매일·매주는 게임 실행 옵션 없이 자동 업데이트만
  저장한다.
- 알림만을 명시 선택하면 기존 `onlyNewVersion` 경로가 유지되며, 레거시
  예약의 동작은 바뀌지 않는다.
- 숨김 Electron 검증과 테스트가 기본 선택, 알림 전환, 재오픈 복원,
  조건부 게임 실행 옵션을 직접 검증한다.
- 전체 테스트의 외부 GGG 공지 길이 assertion 1건은 이번 변경의 blocker가
  아니다.
- 리뷰어는 파일 수정·커밋·테스트 실행 없이 현재 전체 diff와 검증 기록을
  읽기 전용으로 검토했다.
